...
function eventListeners() {
  ...
  btnNew.addEventListener("click", agregarPais);
  miform.addEventListener("submit", guardarPais);
  btnCancelar.addEventListener("click", cancelarPais);
}
...
function guardarPais(e) {
  e.preventDefault();
  const formdata = new FormData(miForm);
  API.savePais(formdata)
    .then((data) => {
      if (data.success) {
        cancelarPais();
        Swal.fire({
...
function crearTabla() {
  if (objDatos.filter == "") {
    ...
    objDatos.recordsFilter = objDatos.records.filter((item) => {
      const { pais } = item;
      if (pais.toUpperCase().search(objDatos.filter.toUpperCase()) != -1) {
        return item;
      }
    });
  }
  ...
  objDatos.recordsFilter.forEach((item, index) => {
    if (index >= recordIni && index <= recordFin) {
      ...
      html += `<tr>
			      ...
			      <th scope="col">
						<button class='btn btn-primary btn-xs' onclick='editarPais(${id_pais})'><i class='far fa-edit'></i></button>
						<button class='btn btn-danger btn-xs' onclick='eliminarPais(${id_pais})'><i class='fas fa-trash-alt'></i></button>
						</th>
			    </tr>`;
    }
  });
...
function limpiarForm(op) {
  miForm.reset();
  document.querySelector("#id_pais").value = "0";
}
